...
Math.PI/180)),p=Math.round(d*Math.sin(c.boxShadowAngle*Math.PI/180)),m=(c.boxShadowLength-d)/c.boxShadowOpacity,m=-d*d/(c.boxShadowOpacity*c.boxShadowLength)+c.boxShadowLength/c.boxShadowOpacity,m=1,n=COLOR_UTILS.transition(h,l,d,c.boxShadowLength);a+=k+"px "+p+"px 0px rgba("+n.r+","+n.g+","+n.b+","+m+"),"}"auto"==c.textShadowAngle&&(c.textShadowAngle=c.boxShadowAngle);l=h=null;"auto"==c.textShadowColor?(d=COLOR_UTILS.parseColor(c.boxColor),k=COLOR_UTILS.RGBToHSB({r:d[0],g:d[1],b:d[2]}),k.b*=1.5,
h=COLOR_UTILS.parseColor("#"+COLOR_UTILS.HSBToHex(k)),l=d):b.isArray(c.textShadowColor)?(h=COLOR_UTILS.parseColor(c.textShadowColor[0]),l=COLOR_UTILS.parseColor(c.textShadowColor[1])):h=l=COLOR_UTILS.parseColor(c.textShadowColor);"auto"==c.textShadowLength&&(c.textShadowLength=c.boxShadowLength);for(d=1;d<=c.textShadowLength;d++)k=Math.round(d*Math.cos(c.textShadowAngle*Math.PI/180)),p=Math.round(d*Math.sin(c.textShadowAngle*Math.PI/180)),m=1,n=COLOR_UTILS.transition(h,l,d,c.textShadowLength),f+=
k+"px "+p+"px 0px rgba("+n.r+","+n.g+","+n.b+","+m+"),";var q=null,r=null;"auto"==c.imgShadowColor?(d=COLOR_UTILS.parseColor(c.boxColor),k=COLOR_UTILS.RGBToHSB({r:d[0],g:d[1],b:d[2]}),k.b*=1.5,q=COLOR_UTILS.parseColor("#"+COLOR_UTILS.HSBToHex(k)),r=d):b.isArray(c.imgShadowColor)?(q=COLOR_UTILS.parseColor(c.imgShadowColor[0]),r=COLOR_UTILS.parseColor(c.imgShadowColor[1])):q=r=COLOR_UTILS.parseColor(c.imgShadowColor);"auto"==c.imgShadowLength&&(c.imgShadowLength=c.boxShadowLength);"auto"==c.imgShadowAngle&&
(c.imgShadowAngle=c.boxShadowAngle);g.each(function(a,d){d.complete&&b.drawImageLongShadow(d,c,q,r)});g.load(function(){b.drawImageLongShadow(this,c,q,r)});for(d=1;d<=c.imgShadowLength;d++);a=a.substring(0,a.length-1);f=f.substring(0,f.length-1);this.css({"box-shadow":a,"-moz-box-shadow":a,"-webkit-box-shadow":a});e.css({"text-shadow":f,"-moz-text-shadow":f,"-webkit-text-shadow":f});return this}});b.drawImageLongShadow=function(a,e,g,f){var d=document.createElement("canvas");d.width=parseInt(e.boxWidth);
d.height=parseInt(e.boxHeight);var c=document.createElement("canvas");c.width=d.width;c.height=d.height;canvasResultCtx=c.getContext("2d");for(var h=d.getContext("2d"),l=e.imgShadowLength;1<=l;l--){h.save();h.clearRect(0,0,d.width,d.height);var k=COLOR_UTILS.transition(g,f,l,e.imgShadowLength),p=Math.round(l*Math.cos(e.imgShadowAngle*Math.PI/180)),m=Math.round(l*Math.sin(e.imgShadowAngle*Math.PI/180));h.translate(p+Math.round(d.width/2-b(a).width()/2),m+Math.round(d.height/2-b(a).height()/2));h.drawImage(a,
0,0);h.globalCompositeOperation="source-in";h.fillStyle="rgb("+k.r+","+k.g+","+k.b+")";h.rect(0,0,d.width,d.height);h.fill();h.restore();canvasResultCtx.drawImage(d,0,0)}b(a).parent().parent().css({"background-image":"none"});e=c.toDataURL("image/png");h.clearRect(0,0,c.width,c.height);canvasResultCtx.clearRect(0,0,d.width,d.height);b(a).parent().parent().css({"background-image":"url("+e+")","background-repeat":"no-repeat","vertical-align":"top"})}})(jQuery);
...
" is not supported by $.parseColor");isNaN(a[3])&&(a[3]=1);return a.slice(0,3)}};
